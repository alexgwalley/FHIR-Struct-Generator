...
void 
AppendStructSerializeFunctionBody(StringBuilder *builder, FhirResource *res, char *argument_name)
{
	ArenaTemp scratch = GetScratch(0);
	...
	StringBuilder_Append(builder, sb_format);
	for(int i = 0; i < res->data.props.count; i++) {
		FhirProperty prop = res->data.props.properties[i];
...
void
AppendStringSerializeFunctionBody(StringBuilder *builder, FhirResource *res, char *argument_name) 
{
	char *sb_format = 
		"\tif(!%s) return false;\n"
...
AppendSerializeFunction(StringBuilder *builder, FhirResource *res)
{
	...
	if(res->type == FhirResourceType_Struct) {
		...
		StringBuilder_Append(builder, "{\n");
		AppendStructSerializeFunctionBody(builder, res, argument_name);
		StringBuilder_Append(builder, "}\n");
	} else if(res->type != FhirResourceType_Unknown){
		AppendStringSerializeFunctionHead(builder, res, argument_name);
		StringBuilder_Append(builder, "{\n");
		AppendStringSerializeFunctionBody(builder, res, argument_name);
		StringBuilder_Append(builder, "}\n");
	}
...
int main() {
	...
	WriteFileContents("../src/fhir_structs_gen.h", builder.string, StringBuilder_GetLength(&builder));
	ReleaseScratch(temp);
	...
	WriteFileContents("../src/fhir_serialize_gen.c", serialize_builder.string, StringBuilder_GetLength(&serialize_builder));
	ReleaseScratch(temp);
