...
HashTable_Add(HashTable *table, String8 key, String8 data)
{
	...
	U64 entry_index = hash.u64[1] % table->count;
	if (table->entries[entry_index].key.size != 0)
	{
		U64 start_index = entry_index;
		entry_index = entry_index+1 % table->count;
...
HashTable_Get(HashTable *table, String8 key)
{
	...
	while (!Str8Match(key, table->entries[entry_index].key, 0) && entry_index != start_index)
	{
		entry_index %= table->count;
	}
